...
def main():
    ...
    with open("cluster_region_probabilities.json", "w", encoding="utf-8") as f:
        json.dump(cluster_probabilities, f, indent=2)

    # Save clusters.txt
    with open("clusters.txt", "w", encoding="utf-8") as f:
        for (image_id, region), cluster in zip(
            zip(labeled_vectors.keys(), labeled_regions),
            cluster_assignments
        ):
            f.write(f"{image_id}\t{region}\t{cluster}\n")

    # Save PNG
    plot_cluster_region_pies(cluster_region_counts, output_png)
